...
namespace CafeManagementSystem.dao
{
    ...
    {
        public ReceiptDetail GetReceiptDetailByProID(int productID)
        {
            ReceiptDetail receiptDetail = null;
            Dao dao = new Dao();
            string sqlStatement = "SELECT ReceiptID, ProductID, Quantity, Total FROM ReceiptDetail WHERE ProductID='" + productID + "'";
            ...
            if (reader.Read())
            {
                ...
                receiptDetail = new ReceiptDetail(rID, pID, q, Decimal.ToDouble(t));
            }
            dao.Con.Close();
